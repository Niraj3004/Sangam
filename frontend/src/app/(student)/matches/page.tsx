                    ...
                    >
                      ...
                      <div className="flex-shrink-0 flex flex-col items-center">
                        <div className="w-24 h-24 rounded-full border-4 border-slate-50 bg-slate-100 flex items-center justify-center text-3xl font-bold text-primary overflow-hidden shadow-sm mb-3">
                          {person.avatarUrl ? (
                            <img src={person.avatarUrl} alt={person.handle} className="w-full h-full object-cover" />
                          ) : (
                            person.handle?.charAt(0).toUpperCase()
                          )}
                        </div>
                        <Link href={`/u/${person.handle}`} className="text-sm font-bold text-foreground hover:text-primary transition-colors flex items-center gap-1">
                          {person.handle}
                          <VerifiedBadge tier={person.userId?.verifyTier || 'unverified'} />
                        </Link>
                      </div>
                      ...
                      <div className="flex-1 flex flex-col pt-2">
                        <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 mb-4 flex-1">
                          ...
                          <p className="text-sm text-indigo-950 font-medium leading-relaxed">
                            {match.reasons?.[0] || match.explanation}
                          </p>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-auto border-t border-border pt-4">
                          ...
                          <button 
                            onClick={() => handleConnect(person.userId?._id)}
                        ...
                        <div className="flex items-center justify-between border-t border-border pt-4 mt-auto">
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-semibold text-foreground flex items-center gap-1">
                              Lead by {project.ownerId?.email?.split('@')[0] || 'Unknown'}
                            </span>
                          </div>
                          <Link 
