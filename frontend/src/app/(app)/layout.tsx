...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { name: "Discover People", href: "/discover", icon: <Users className="w-5 h-5" /> },
    { name: "Ideas Board", href: "/ideas", icon: <Lightbulb className="w-5 h-5" /> },
    { name: "Projects & Teams", href: "/projects", icon: <Briefcase className="w-5 h-5" /> },
    ...
    <div className="min-h-screen bg-secondary flex">
      ...
      <aside className="w-64 bg-white border-r border-border flex flex-col fixed h-full z-20">
        ...
        <div className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          <div className="px-3 mb-2 text-xs font-semibold text-muted uppercase tracking-wider">Main</div>
          {navItems.map((item) => {
